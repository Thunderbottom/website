---
title: "Setting up Void Linux with Full-Disk Encryption"
...
This guide serves as an extension to the official Void documentation for Full Disk Encryption<Sidenote id="1">Check out the [Void Linux: Full Disk Encryption](https://docs.voidlinux.org/installation/guides/fde.html) wiki</Sidenote> in an attempt to make the steps easier to follow.
...
We then need to add the filesystems entries to `/mnt/etc/fstab`. You are expected to responsibly replace all the values in the configuration below, as per your system's configuration.
...
We now need to `chroot`<Sidenote id="4">[chroot: arch wiki](https://wiki.archlinux.org/index.php/Chroot)</Sidenote> into our newly installed system and set up the bootloader.

```console
$ chroot /mnt /bin/bash
```

PThis should put you in a shell inside your newly installed system.
